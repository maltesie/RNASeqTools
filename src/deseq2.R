...
files <- list.files(path = input_path, pattern = "*.csv", full.names = TRUE, recursive = FALSE)
...
lapply(files, function(x) {
    ...
    num_replicates <- as.integer(args[3])
    if (length(colnames(countdata)) == (2 * num_replicates)) {
        (condition <- factor(c(rep("ctl", num_replicates), rep("exp", num_replicates))))
